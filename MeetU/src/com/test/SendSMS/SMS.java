...
public class SMS
{
    ...
    static final String MODULE_VERSION = "Java/2.3.1";
    ...
    public HashMap<String,String> cancel(String messageId) throws IOException
    {
		if (this.userid == null || this.password == null)
		{
			System.out.println("setuser required");
        ...
        byte[] packet = encodeCancel(messageId);
        fos.write(packet, 0, packet.length);
        fos.flush();
    ...
    public SmsBalanceInfo getBalanceInfo() throws IOException
    {
        HashMap<String,String> res = remain();
        SmsBalanceInfo sbi = new SmsBalanceInfo();
        sbi.cash = Integer.parseInt(res.get("CASH"));
        sbi.point = Integer.parseInt(res.get("POINT"));
        sbi.mdrop = Integer.parseInt(res.get("DROP"));
        sbi.credits = Integer.parseInt(res.get("CREDITS"));
        sbi.resultCode = res.get("RESULT-CODE");
        sbi.resultMessage = res.get("RESULT-MESSAGE");
	...
	private byte[] encodeCancel(String messageId) throws IOException
	{
		HashMap<String, String> map = new HashMap<String, String>();
		...
		if (messageId != null) map.put("MESSAGE-ID", messageId);
		if (this.crypt.equals("MD5")) map.put("CRYPT-METHOD", "MD5");
        if (this.userid != null) map.put("AUTH-ID", this.userid);
		if (this.password != null) map.put("AUTH-PASS", this.password);
		map.put("LANGUAGE", "JAVA");
